<template>
  <input
    ...
    :checked="checked"
  />
  <label for="checkbox" style="position: absolute; top: 8px; right: 8px">
    <svg
      id="heart-svg"
...
export default {
  ...
  methods: {
    ...
    clickCheckbox() {
      ...
      if (this.checked) {
        this.registerLikedApt(requestBody);
      } else {
        this.deleteLikedApt(requestBody);
      }
      this.getLikedAptList(this.checkUserInfo.id);
    },
  },
  computed: {
    ...mapGetters(aptStore, ["isLikedApt", "curApt"]),
    ...mapGetters(userStore, ["checkUserInfo"]),
  },
  watch: {
    isLikedApt() {
      this.checked = this.isLikedApt === "success" ? true : false;
    },
  },
  created() {
    this.checked = this.isLikedApt === "success" ? true : false;
